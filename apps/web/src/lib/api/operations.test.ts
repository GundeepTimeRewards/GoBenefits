...
describe("C1 operation registry", () => {
  test("exposes the C1 operations + Phase D reads/mutations + E-1 elections review", () => {
    expect(C1_OPERATION_NAMES.length).toBe(63);
    for (const name of [
      ...
      "createPlanYear", "copyFromPriorYear", "activatePlanYear", "archivePlanYear",
      ...
      "lifeEventQueue", "approveLifeEvent", "denyLifeEvent", "requestLifeEventDocs", "openElectionWindow",
      ...
      "uploadDocument", "requestSignature", "reportLifeEvent", "employeeLifeEvents", "planComparison", "askBenefitsAssistant",
    ]) {
      expect(C1_OPERATION_NAMES).toContain(name as any);
    }
  });

  test("kinds are correct (23 queries, 40 mutations)", () => {
    const kinds = C1_OPERATION_NAMES.map((n) => operations[n].kind);
    expect(kinds.filter((k) => k === "query").length).toBe(23);
    expect(kinds.filter((k) => k === "mutation").length).toBe(40);
  });
